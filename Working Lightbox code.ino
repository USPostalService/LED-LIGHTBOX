...
#define __brightness_levels 25 // 0...15 above 28 is bad for ISR ( move to timer1, lower irq freq ! )
...
for (counter1 = 0; counter1 <= 7; counter1++) {
  for (counter2 = 0; counter2 <= 7; counter2++) {
    ...
    delay(10);
    set_led_rgb(counter1,counter2,0,0,200);
    delay(10);
    set_led_rgb(counter1,counter2,50,100,200);
    delay(10);
  }
}
...
ISR(TIMER1_OVF_vect) { /* Framebuffer interrupt routine */
  ...
  for(cycle = 0; cycle < __max_brightness; cycle++) {
    byte led;
    byte row = B0000000;	// row: current source. on when (1)
